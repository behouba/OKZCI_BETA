...
function firstSearch(q, cat, city, st) {
    adsField.style.display = 'none'
    homeSpinner.style.display = 'block'
    console.log(q, cat, city, st)
    let url = '/search?query=' + q + '&category=' + cat + '&city=' + city + '&sort=' + st
    axios.get(url)
        .then(res => {
            adsField.innerHTML = ''
            if (res.data) {
                ...
                adsField.innerHTML = 'Rien a été trouvé pour cette recherche desolé !'
...
searchBar.addEventListener('submit', e => {
    ...
    firstSearch(search.value, category, city, sort)
    clearSearch()
})
